...
def check_for_errors(zip_file_path):
    ...
    try:
        ...
        if len(extracted_items) != 1 or not os.path.isdir(os.path.join(temp_dir, extracted_items[0])):
            print(f"{COLOUR.RED}Error: The .zip file should contain exactly one parent folder.{COLOUR.STOP}")
            return
        ...
        match = re.search(r'_(Insert|Update|Takedown)_', batch_name)
        if match:
        ...
        for subdir_name in tqdm(subdirs, desc="Analyzing batch files... ", unit=" folder", bar_format="{l_bar}{bar:30}", ncols=75):
        ...
        if batch_type != "Takedown":                
            for subdir_name in tqdm(subdirs, desc="Checking for corrupted JPEGs... ", unit=" folder", bar_format="{l_bar}{bar:30}", ncols=75):
                subdir_path = os.path.join(parent_folder_path, subdir_name)
                res_dir = os.path.join(subdir_path, "resources")
        ...
        if error_details:
            ...
            print(f"{COLOUR.YELLOW}Removing broken releases from batch {batch_name}...{COLOUR.STOP}")
            move_broken_release_errors = {}
            for error in error_details:
                try:
                    shutil.move(os.path.join(parent_folder_path, error['folder_name']), failed_releases_folder)
                except FileExistsError as e:
                    move_broken_release_errors[error]
            ...
            while True:
                ...
                if create_cleaned == 'y' or create_cleaned == 'n':
                    break
                else:
            ...
            if create_cleaned == 'y':
                ...
                with zipfile.ZipFile(cleaned_zip_path, 'w', zipfile.ZIP_DEFLATED) as cleaned_zip:
                    for file in tqdm(files_to_zip, desc="Creating cleaned zip", unit="file", bar_format="{l_bar}{bar:30}", ncols=75):
                ...
                if ask_delete == True:
                    ...
                    if delete_original == "n":
                        print(f"Orignial batch '{batch_name}' not deleted.")
        ...
        print(f"{COLOUR.RED}Zip file is invalid! Please check the zip file and re-download batch {os.path.basename(zip_file_path)} from the hub.{COLOUR.STOP}")

    # Clean up temporary directory
    finally:
        if os.path.isdir(temp_dir):
            shutil.rmtree(temp_dir)
            print(f"Temp folder '{temp_dir}' deleted.")
